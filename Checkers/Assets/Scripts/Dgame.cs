...
public class Dgame : MonoBehaviour
{
    ...
    void createPiece(int n, int m, int oddRow = 0, bool isBrown = true)
    {
        ...
        if (isBrown)
        {
            GameObject cylinder = Instantiate(whitePiece, new Vector3(0 + 2 * n + offsetx + oddRow, 0.35f, +m + offsetz), Quaternion.identity);
            cylinder.transform.rotation = Quaternion.Euler(-90f, 0f, 0f);
            cylinder.transform.localScale = new Vector3(50f, 50f, 10f);
            cylinder.name = "WhitePiece";
        ...
        else
        {
            ...
            GameObject cylinder = Instantiate(blackPiece, new Vector3(0 + 2 * n + offsetx + oddRow, 0.35f, +m + offsetz), Quaternion.identity);
            cylinder.transform.rotation = Quaternion.Euler(-90f, 0f, 0f);
            cylinder.transform.localScale = new Vector3(50f, 50f, 10f);
            cylinder.name = "BlackPiece";
    ...
    void createBoard(int row, int col, bool isRed = false)
    {
        ...
        // Instantiate(boardObj, new Vector3(0,0,0), new Quaternion(45,0,0,0));
        // boardObj.transform.Rotate(90.0f, 0.0f, 0.0f, Space.Self);
    }
    void setGame()
    {
        ...
        GameObject temp = Instantiate(boardObj, new Vector3(-0.5f, 0, -0.5f), Quaternion.identity);
        temp.transform.rotation = Quaternion.Euler(-90f, 0f, 180f);
        temp.transform.localScale = new Vector3(500f, 500f, 0.5f);
    ...
    void getCurrentAvailableMoves(int currentpiece_x, int currentpiece_y)
    {
        ...
        foreach (var move in moves)
        {
            if (move[0] == currentpiece_x && move[1] == currentpiece_y)
            {
                ...
                movespot.transform.position = new Vector3(move[3] + offsetx, 0.1f, move[2] + offsetz);
                movespot.name = "movespot";
                movespot.transform.localScale = new Vector3(0.08f, 0.005f, 0.08f);
    ...
    void makeaMove(int old_i, int old_j, int new_i, int new_j, int queen_row)
    {
        ...
        if (new_i == queen_row)
        {
            if (player < 3)
            {
                ...
                {
                Destroy(piece);
                    piece = Instantiate(whiteKing, new Vector3(new_j + offsetx , 0.35f, new_i + offsetz ), Quaternion.identity);
                    piece.transform.localScale = new Vector3(50f, 50f, 10f);
                    piece.name = "WhitePiece";
                    piece.transform.rotation = Quaternion.Euler(-90f, 0f, 180f);
                    playerPieces.Add(piece);
                }
                else
                {
                Destroy(piece);
                    piece = Instantiate(blackKing, new Vector3(new_j + offsetx, 0.35f, new_i + offsetz), Quaternion.identity);
                    piece.transform.localScale = new Vector3(50f, 50f, 10f);
        ...
        if (movea.name=="WhitePiece" || movea.name == "BlackPiece"){
            movea.transform.position = new Vector3(new_j + offsetx, 0.35f, new_i + offsetz);
            Debug.Log("is it rong here?"+movea+movea.transform.position);}
    ...
    {
        if (GetItemAtCoordinate(x, y).name == "WhitePiece" || GetItemAtCoordinate(x, y).name == "WhiteKing")
            whitePieces--;
        else
            redPieces--;
    ...
    void playerMove()
    {
        ...
        if (selectedPiece)
        {
            if (selectedPiece.name == "WhitePiece")
            {
